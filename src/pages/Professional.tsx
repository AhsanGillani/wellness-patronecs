...
import { useProfessionals, useProfessionalFeedback, useProfessionalServices } from "@/hooks/useDatabase";
...
const ServicesTabs = ({ services = [], profId, prof, feedback, servicesLoading, servicesError }: { 
  services?: Service[]; 
  profId: string; 
  prof: any; 
  feedback: any[];
    ...
    <div>
      <div className="flex items-center gap-2 border-b px-4 pt-4">
        {(["services", "reviews"] as const).map((tab) => (
          ...
          >
            {tab === "services" ? `Services (${services.length})` : `Reviews (${feedback.length})`}
          </button>
        ))}
      </div>
        ...
        <div className="p-6">
          {servicesLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-semibold text-slate-900 mb-2">Error loading services</h3>
                ...
                <div key={s.id} className="rounded-xl border p-4">
                  <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="font-medium text-slate-900">{s.name}</div>
                        <div className="flex items-center gap-3 mt-1">
                          <div className="text-sm text-slate-600">{s.duration_min} min</div>
                      ...
                      <div className="text-base font-semibold text-slate-900">${(s.price_cents / 100).toFixed(2)}</div>
                  </div>
                  ...
                  <div className="mt-3 flex gap-2">
                      <Button 
                        ...
                        size="sm"
                        onClick={() => console.log('View Details clicked - prof.slug:', prof.slug, 'serviceSlug:', serviceSlug, 'URL:', `/services/${prof.slug}/${serviceSlug}`)}
                      >
                        View Details
                      </Button>
                      <Button 
            ...
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-semibold text-slate-900 mb-2">No services listed</h3>
              <p className="text-slate-600">This professional hasn't added any services yet.</p>
            </div>
          )}
        </div>
      )}
                ...
                <div key={review.id} className="border-b border-slate-100 pb-6 last:border-b-0">
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0">
                      <img 
                        src={review.profiles?.avatar_url || "https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?w=400"} 
                        alt={`${review.profiles?.first_name || 'Patient'} ${review.profiles?.last_name || ''}`}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {review.feedback_text && (
                        <p className="text-slate-700 mb-2">{review.feedback_text}</p>
                      )}
                      {review.additional_comments && (
                        <p className="text-sm text-slate-600 italic">"{review.additional_comments}"</p>
                      )}
...
const Professional = () => {
  ...
  // Fetch feedback for this professional
  const { feedback, loading: feedbackLoading, error: feedbackError } = useProfessionalFeedback(prof?.id);
  
  // Fetch services for this professional
  const { services, loading: servicesLoading, error: servicesError } = useProfessionalServices(prof?.id);
      ...
      <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
        ...
        <main className="py-16">
          <div className="mx-auto max-w-3xl px-4 text-center">
            ...
            <p className="mt-2 text-slate-600">Loading professional profile...</p>
          </div>
        </main>
        <Footer />
      </div>
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
        ...
        <main className="py-16">
          <div className="mx-auto max-w-3xl px-4 text-center">
            <h1 className="text-2xl font-bold text-slate-900">Error Loading Profile</h1>
          ...
          <section className="lg:col-span-8">
            <div className="rounded-2xl border bg-white p-6">
              <div className="flex flex-col sm:flex-row sm:items-center gap-5">
                <img 
                  src={prof.avatar_url || prof.image || "https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?w=400"} 
                  alt={prof.name} 
                ...
                <div className="w-full sm:w-auto sm:text-right">
                  ...
                  <div className="text-base font-semibold text-slate-900">
                    {prof.price || (prof.price_per_session !== null && prof.price_per_session !== undefined ? `$${(prof.price_per_session / 100).toFixed(0)}` : '$50')}
                  </div>
            ...
            <div className="mt-6 space-y-6">
              <div className="rounded-2xl border bg-white p-6">
                ...
                <p className="mt-2 text-slate-700 leading-relaxed">
                  {prof.about || prof.bio || `${prof.name} is a ${prof.title} with ${prof.years} years of experience in ${prof.specialization || 'their field'}.`}
                </p>
              </div>

            <div className="rounded-2xl border bg-white p-0 overflow-hidden">
              <ServicesTabs services={services || []} profId={prof.id} prof={prof} feedback={feedback || []} servicesLoading={servicesLoading} servicesError={servicesError} />
            </div>
            ...
            <div className="rounded-2xl border bg-white p-6">
              ...
              <div className="mt-2 text-slate-700">
                {feedback && feedback.length > 0 ? (
                  <>
                    Rated {prof.rating} by {feedback.length} patient{feedback.length !== 1 ? 's' : ''}
                    {feedbackError && (
                  ...
                  <>
                    ...
                    {feedbackLoading && (
                      <div className="text-sm text-slate-500 mt-1">Loading reviews...</div>
                    )}
                  </>
                )}
              </div>
                    ...
                    <div className="text-sm text-slate-700 mt-1">
                      {prof.location ? (
                        <span className="inline-flex items-center gap-1">
                          {prof.location}
                          {prof.location.includes('Virtual') || prof.location.includes('Online') ? (
              ...
              {services && services.length > 0 && (
                <div className="mt-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-sm font-medium text-slate-900">Service Modes</div>
                      <div className="flex flex-wrap gap-2 mt-1">
                        {Array.from(new Set(services.map(s => s.mode))).map((mode) => (
                          <span key={mode} className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
                            mode === 'Virtual' ? 'bg-blue-100 text-blue-700' :
                            mode === 'In-person' ? 'bg-green-100 text-green-700' :
                ...
                <div className="mt-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-1">
                        {Array.from(new Set(services.map(s => s.categories?.name).filter(Boolean))).map((category) => (
                          <span key={category} className="inline-flex items-center rounded-full bg-violet-100 px-2 py-0.5 text-xs font-medium text-violet-700">
                            {category}
                          </span>
                        ))}
                      </div>
                      ...
                      {services && services.length > 0 ? (
                        <div className="space-y-1">
                          ...
                          <div className="text-xs text-slate-500">
                            {services.filter(s => s.mode === 'Virtual').length > 0 && 
                              `${services.filter(s => s.mode === 'Virtual').length} virtual service${services.filter(s => s.mode === 'Virtual').length !== 1 ? 's' : ''} available`
                            }
                          </div>
                          <div className="text-xs text-slate-500">
                            {services.filter(s => s.mode === 'In-person').length > 0 && 
                              `${services.filter(s => s.mode === 'In-person').length} in-person service${services.filter(s => s.mode === 'In-person').length !== 1 ? 's' : ''} available`
                            }
                          </div>
                        </div>
                      ) : (
