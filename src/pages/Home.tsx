...
const Home = () => {
  const { data: pros = [], isLoading: prosLoading } = useProfessionals();
  const { data: blogs = [], isLoading: blogsLoading } = useBlogs();
  const { profile } = useAuth();
  const topPros = (pros || []).slice(0, 3);
  const avatars = [avatar1, avatar2, avatar3];
  const shouldShowStripeBanner = Boolean(
    profile &&
      profile.role === "professional" &&
      !(profile as { stripe_account_id?: string }).stripe_account_id
  );
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
        ...
        <div className="bg-amber-50 border-b border-amber-200 text-amber-900">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-2 text-sm flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-amber-200 text-amber-800 text-xs">
                !
              </span>
              <span>
                Professionals: Connect your Stripe account to receive payouts.
              </span>
            </div>
            <Link
              to="/doctor-dashboard?tab=billing"
              className="underline text-amber-900 hover:text-amber-800"
            >
              Connect now
            </Link>
          </div>
      ...
      <main>
        ...
        <section className="relative overflow-hidden">
          ...
          {/* Floating elements */}
          <div
            className="absolute top-20 left-10 w-20 h-20 bg-violet-200/30 rounded-full blur-xl animate-pulse"
            style={{ animationDelay: "0s" }}
          ></div>
          <div
            className="absolute top-40 right-20 w-32 h-32 bg-blue-200/20 rounded-full blur-xl animate-pulse"
            style={{ animationDelay: "2s" }}
          ></div>
          <div
            className="absolute bottom-40 left-1/4 w-16 h-16 bg-emerald-200/30 rounded-full blur-xl animate-pulse"
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                ...
                <p className="text-slate-600 text-base sm:text-lg leading-relaxed max-w-lg">
                  Book appointments, get answers, and discover curated wellness
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              ...
              <p className="text-lg text-slate-600 max-w-2xl mx-auto">
                Discover our comprehensive platform designed to connect you with
                ...
                <div key={item.title} className="group relative">
                  ...
                  <Link to={item.link} className="block">
                    <div className="relative rounded-2xl border bg-white p-8 shadow-soft hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer">
                      <div
                        className={`h-16 w-16 rounded-2xl ${item.bgColor} text-4xl grid place-items-center mb-6 group-hover:scale-110 transition-transform duration-300`}
                      >
                        {item.icon}
                      </div>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="rounded-2xl border bg-white p-6">
                ...
                <div className="mt-2 text-slate-900 font-medium">
                  Patients report easy access to care when they need it.*
                </div>
              ...
              <div className="rounded-2xl border bg-white p-6">
                ...
                <ul className="mt-2 list-disc pl-5 text-sm text-slate-600 space-y-1">
                  <li>
                    Find the right professional in seconds with smart matching
                  </li>
                  <li>
                    Book in minutes — secure telehealth or in‑person sessions
                  </li>
                  <li>
                    All chats, notes and follow‑ups in one HIPAA‑grade place
                  </li>
                </ul>
              </div>
              ...
              <div className="rounded-2xl border bg-white p-6 md:col-span-2">
                <div className="text-4xl sm:text-5xl font-extrabold text-violet-700">
                  <CountUp end={100} />{" "}
                  <span className="align-baseline">Million</span>
                </div>
              ...
              <div className="rounded-2xl border bg-white p-6">
                ...
                <div className="mt-2 text-slate-900">
                  of people with depression feel better after their third visit
              ...
              {!prosLoading &&
                topPros.map((p, idx) => {
                  ...
                  return (
                    <div
                      key={p.slug || p.id}
                      className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-lg hover:border-violet-200 transition-all duration-300 hover:-translate-y-1"
                    >
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {(blogsLoading ? [1, 2, 3] : []).map((i) => (
                <div
                  key={`blog-skel-${i}`}
                  className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-lg transition-all duration-300"
                >
              ...
              {!blogsLoading && blogs.length === 0 && (
                <div className="col-span-full text-center py-12">
                  ...
                  <p className="text-slate-600 mb-4">
                    We're working on some amazing wellness content for you.
                  </p>
              ...
              {!blogsLoading &&
                blogs.slice(0, 3).map((post, idx) => {
                  const cover =
                    post.cover_url || [article1, article2, article3][idx % 3];
                  const tag =
                  ...
                  return (
                    <Link
                      key={post.slug}
                      to={`/blogs/${post.slug}`}
                      className="group block overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-lg hover:border-violet-200 transition-all duration-300 hover:-translate-y-1"
                    >
                      {/* Image container */}
              ...
              <div className="grid lg:grid-cols-2 gap-6 items-center">
                <div>
                  ...
                  <p className="mt-2 text-white/80">
                    Join thousands who trust our platform to find the care they
                    need.
                  </p>
                </div>
                <div className="flex gap-3 lg:justify-end">
                  <a
                    href="#"
                    className="inline-flex items-center justify-center rounded-md bg-white px-5 py-2.5 text-violet-700 font-medium hover:bg-white/90"
                  >
                    Create account
                  </a>
                  <a
                    href="#"
                    className="inline-flex items-center justify-center rounded-md border border-white/20 px-5 py-2.5 text-white font-medium hover:bg-white/10"
                  >
                    Contact us
                  </a>
                </div>
